...
function ProgressSizeCell({ column, row }: ComponentMatrixCellProps) {
  return (
    <Progress
      className="w-48"
      size={row.key as 'lg' | 'md' | 'sm'}
      value={Number(column.key)}
    />
  );
}
...
export const All: StoryDefinition = {
  ...
  render: () => (
    <Story.Layout className="max-w-5xl" title="Progress">
      <Story.Section title="Sizes and values">
        <Story.Matrix
          Cell={ProgressSizeCell}
      ...
      <Story.Section title="Labeled progress">
        <YStack gap={6}>
          ...
          <Progress value={null}>
            <Progress.Track>
              <Progress.Indicator className="bg-muted-foreground/40 w-1/3 animate-pulse" />
            </Progress.Track>
          </Progress>
          <Text tone="muted" variant="small">
            Indeterminate progress uses a null value.
          </Text>
        </YStack>
      </Story.Section>
